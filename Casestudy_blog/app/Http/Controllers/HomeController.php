...
class HomeController extends Controller
{
    ...
    public function getContent($tit){
    	$id = "";
        $new = DB::table('news')->where('changetitle','=',$tit)->limit(5)->get();
        foreach($new as $val){
            $id = $val->id;
            $n = news::find($id);
            $n['view'] += 1;
    ...
    public function getCategory($cate){
        $new = news::where('category_id','=',$cate)->get();
        $category = cate::find($cate);

        return view('home.cate',['new' => $new , 'category' => $category]);
    }

    public function getSearch(Request $req){
        $new = news::where('title','like','%'.request('key').'%')->latest()->paginate(5);


        return view('home.search',compact('new'));
    }
